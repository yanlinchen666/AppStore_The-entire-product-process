...
class PRDService:
    ...
    def generate_requirements(self, db: Session, run_id: int, findings: List[Dict]) -> List[Dict]:
        findings_text = "\n".join([
...
"""
    ...
    def save_prd(self, db: Session, run_id: int, requirements: List[Dict], versions: List[Dict]) -> None:
        ...
        for req_data in requirements:
            requirement = PRDRequirement(
                run_id=run_id,
                finding_id=req_data.get('source_findings', [None])[0] if req_data.get('source_findings') else None,
    ...
    def generate_prd(self, db: Session, run_id: int) -> Dict[str, Any]:
        ...
        requirements = self.generate_requirements(db, run_id, findings)
